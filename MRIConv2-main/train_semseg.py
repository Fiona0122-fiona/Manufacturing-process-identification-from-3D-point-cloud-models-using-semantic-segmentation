...
import argparse
import os
from data_utils.MRICONV2DataLoader import MRICONV2Dataset
import torch
import datetime
import logging
...
def main(args):
    ...
    TRAIN_DATASET = MRICONV2Dataset(split='train', data_root=root, num_point=NUM_POINT, test_group=args.test_group, block_size=1325, sample_rate=1.0, transform=None)
    print("start loading test data ...")
    TEST_DATASET = MRICONV2Dataset(split='test', data_root=root, num_point=NUM_POINT, test_group=args.test_group, block_size=1325, sample_rate=1.0, transform=None)
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, (points, target) in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.8):
            optimizer.zero_grad()
